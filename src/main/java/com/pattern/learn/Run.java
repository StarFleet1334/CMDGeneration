package com.pattern.learn;

import com.pattern.learn.class_creation.ClassCreator;
import com.pattern.learn.utility.DirectoryLister;

import java.util.Scanner;

public class Run {

    private static final Scanner scanner = new Scanner(System.in);
    private static final ClassCreator classCreator = new ClassCreator();

    private static final String TXT = """
        List of options you can create:
            1. Class -> Press '1'
            2. Interface -> Press '2'
            3. Enum -> Press '3'
    
    """;

    public static void main(String[] args) {
        System.out.println(TXT);
        int number = scanner.nextInt();
        if (number == 1) {
            System.out.println("Here are List of Directories, where you can create the class: ");
            for (int i = 0; i < DirectoryLister.getDirectories().size(); i++) {
                System.out.println("Index: " + i + ", " + DirectoryLister.getDirectories().get(i));
            }
            System.out.println("Choose: ");
            int index = scanner.nextInt();
            classCreator.createJavaClass(DirectoryLister.getDirectories().get(index));
        }

    }
}
